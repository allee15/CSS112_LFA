...
def verificare_egalitate(tape_track1,tape_track2): #tape_track1 si tape_track2 sunt 2 liste (reprezinta cele 2 tape-uri ale TM)
    #le verificam ca pe 2 array-uri, cu 2 indici care pleaca de la 0 (de la primul element din fiecare lista), indicii cresc simultan si 
    #presupunem ca cele 2 tape-uri sunt identice si cautam minim o diferenta (daca aceasta exista)
    #daca nu gasim diferente => cele 2 tape-uri sunt egale si returnam 1
    i=0
    j=0
    while i<len(tape_track1) and j<len(tape_track2):
        if i!=j:
            return 0
        i+=1
        j+=1
                ...
                for i in range(len(tranzitii)):
                    if tranzitii[i][0] == curent_state and tranzitii[i][2] == lit:
                        ...
                        if (curent_state == 'reject'):#daca ajungem in nodul de reject => string-ul dat nu este valid
                            ...
                            break
                        break
            if curent_state == 'accept':
                print("String acceptat") #daca ajungem aici=> stringul din input este valid si incepem verificarea celor 2 tape-uri, 
                #sa vedem daca sunt egale; pentru a afla acest lucru ne vom folosi de un subprogram care returneaza 1 (True) sau 0 (False)
                if verificare_egalitate(tape_track1, tape_track2) == 1:
                    print("Cele 2 tape-uri sunt identice")
                else:
                    print("Cele 2 tape-uri nu sunt identice")